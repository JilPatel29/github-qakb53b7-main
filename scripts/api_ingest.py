...
_MITRE_FALLBACKS_BY_TYPE = {
    'IP': ('T1133 - External Remote Services', 'Initial Access'),
    'domain': ('T1583 - Acquire Infrastructure', 'Resource Development'),
    'hash': ('T1027 - Obfuscated Files or Information', 'Defense Evasion'),
    'url': ('T1189 - Drive-by Compromise', 'Initial Access'),
}
...
def _get_mitre(threat_category: str, indicator_type: str = 'IP'):
    mapping = MITRE_MAPPINGS.get(threat_category)
    if mapping:
        return mapping
    return _MITRE_FALLBACKS_BY_TYPE.get(indicator_type, ('T1071 - Application Layer Protocol', 'Command and Control'))
...
class ThreatIngestor:
    ...
    def map_to_mitre(self, indicator, indicator_type, threat_category, risk_level):
        technique, tactic = _get_mitre(threat_category, indicator_type)
        self.cursor.execute("""
